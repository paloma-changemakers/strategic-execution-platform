...
const northStarData = [
  { week: "W1", value: 1200 }, { week: "W2", value: 1350 },
  { week: "W3", value: 1280 }, { week: "W4", value: 1520 },
  { week: "W5", value: 1680 }, { week: "W6", value: 1750 },
  { week: "W7", value: 1900 }, { week: "W8", value: 2100 },
];
...
const initiatives = [
  {
    name: "MVP Launch — Cohort 3", owner: "Ana R.", due: "May 15", progress: 85, status: "In Progress",
  ...
  {
    name: "Investor Deck Q2", owner: "Carlos M.", due: "May 8", progress: 60, status: "In Progress",
  ...
  {
    name: "User Research Sprint", owner: "Paloma Q.", due: "Apr 30", progress: 100, status: "Completed",
  ...
  {
    name: "Partnership Program", owner: "Luis T.", due: "Jun 1", progress: 20, status: "Blocked",
    objective: "Grow active user base",
  ...
  return (
    <Modal open title={initiative.name} onClose={onClose} width={580}>
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        <Badge variant={initiative.status === "Completed" ? "success" : initiative.status === "Blocked" ? "danger" : "info"}>{initiative.status}</Badge>
        <Badge variant="neutral">{initiative.objective}</Badge>
        <Badge variant="neutral">Due {initiative.due}</Badge>
      </div>
      <p style={{ fontSize: 13, color: "#475569", lineHeight: 1.6, marginBottom: 20 }}>{initiative.description}</p>
      {initiative.blockers && (
        <div style={{ padding: "12px 14px", borderRadius: 9, background: "#fff1f2", border: "1px solid #fecdd3", marginBottom: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 20 }}>
        {[
          ...
          <div key={i} style={{ padding: "10px 12px", background: "#f8fafc", borderRadius: 8, border: "1px solid #f1f5f9" }}>
            <div style={{ fontSize: 10, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>{stat.label}</div>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {initiative.projects.map((proj, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "#fafafa", borderRadius: 7, border: "1px solid #f1f5f9" }}>
...
export default function DashboardPage() {
  ...
  return (
    <div className="page-padding" style={{ padding: 28 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 className="page-title" style={{ fontSize: 22, fontWeight: 700, color: "#0f172a", letterSpacing: "-0.02em" }}>Dashboard</h1>
        <p style={{ color: "#64748b", fontSize: 13, marginTop: 2 }}>Future Founders Academy · Week of April 28, 2026</p>
      </div>
      ...
      <div className="grid-3-1" style={{ marginBottom: 16 }}>
        {/* Health Score */}
        <Card>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 16, textTransform: "uppercase", letterSpacing: "0.06em" }}>Strategic Health</div>
        ...
        <Card>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em" }}>North Star Metric</div>
              <div style={{ fontSize: 14, color: "#0f172a", fontWeight: 700, marginTop: 3 }}>Weekly Active Founders</div>
            </div>
        ...
        {/* Alerts */}
        <Card>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.06em" }}>Active Alerts</div>
        ...
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {initiatives.map((init, i) => (
              <div key={i} onClick={() => setSelectedInitiative(init)}
                style={{ padding: "12px 14px", borderRadius: 10, border: "1px solid #f1f5f9", background: "#fafafa", cursor: "pointer" }}
                onMouseEnter={e => { e.currentTarget.style.border = "1px solid #c7d2fe"; e.currentTarget.style.background = "#f5f3ff"; }}
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  <div style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{init.name}</div>
                    <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{init.owner} · Due {init.due}</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                    <Badge variant={init.status === "Completed" ? "success" : init.status === "Blocked" ? "danger" : "info"}>{init.status}</Badge>
      ...
      {selectedInitiative && (
        <InitiativeDetailModal initiative={selectedInitiative} onClose={() => setSelectedInitiative(null)} />
      )}
    </div>
  );
}
